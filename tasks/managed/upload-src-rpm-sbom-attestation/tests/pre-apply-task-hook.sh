...
kubectl delete configmap test-mocks-upload-sbom-att --ignore-not-found
kubectl create configmap test-mocks-upload-sbom-att --from-file=mocks.sh="$SCRIPT_DIR/mocks.sh"
yq -i '.spec.volumes += [{"name": "test-mocks", "configMap": {"name": "test-mocks-upload-sbom-att"}}]' "$TASK_PATH"
