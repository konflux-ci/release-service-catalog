...
kubectl create secret generic pulp-task-exodus-secret --from-literal=cert=SENSITIVE_DATA_myexoduscert --from-literal=key=SENSITIVE_DATA_myexoduskey --from-literal=url=https://SENSITIVE_DATA_exodus.com

# Create a dummy pulp secret (and delete it first if it exists)
kubectl delete secret pulp-task-pulp-secret --ignore-not-found
kubectl create secret generic pulp-task-pulp-secret --from-literal=konflux-release-rhsm-pulp.crt=SENSITIVE_DATA_mypulpcert --from-literal=konflux-release-rhsm-pulp.key=SENSITIVE_DATA_mypulpkey --from-literal=pulp_url=https://SENSITIVE_DATA_pulp.com
...
kubectl create secret generic pulp-task-bad-pulp-secret --from-literal=konflux-release-rhsm-pulp.crt=SENSITIVE_DATA_mypulpcert --from-literal=konflux-release-rhsm-pulp.key=SENSITIVE_DATA_mypulpkey --from-literal=pulp_url=https://SENSITIVE_DATA_failing-pulp.com

# Create a dummy udc secret (and delete it first if it exists)
kubectl delete secret pulp-task-udc-secret --ignore-not-found
kubectl create secret generic pulp-task-udc-secret --from-literal=cert=SENSITIVE_DATA_myudccert --from-literal=key=SENSITIVE_DATA_myudckey --from-literal=url=https://SENSITIVE_DATA_udc.com
