...
# Return the Succeeded condition status for a pipeline task's TaskRun.
# Args: pipelinerun_name, pipeline_task_label
get_managed_task_succeeded_status() {
  local pipelinerun_name=$1
  local pipeline_task=$2
  ...
  kubectl get taskrun "${taskrun_name}" -n "${managed_namespace}" \
...
EOF
  ...
  local sign_task_succeeded
  sign_task_succeeded=$(get_managed_task_succeeded_status "${second_pipelinerun_name}" "sign-checksum-blob")
  if [ "${sign_task_succeeded}" == "True" ]; then
