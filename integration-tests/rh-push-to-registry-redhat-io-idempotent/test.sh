...
verify_single_release() {
    ...
    local skopeo_max_attempts=3
    local skopeo_ok=false
    for ((skopeo_attempt=1; skopeo_attempt<=skopeo_max_attempts; skopeo_attempt++)); do
        ...
        if [ $skopeo_attempt -lt $skopeo_max_attempts ]; then
            echo "⚠️  skopeo inspect failed (attempt ${skopeo_attempt}/${skopeo_max_attempts}), retrying in 30s..."
            sleep 30
        fi
    done
    if [ "${skopeo_ok}" != "true" ]; then
        echo "🔴 Failed to pull or inspect image '$COMPLETE_PULLSPEC'."
        skopeo inspect --tls-verify=true "docker://${COMPLETE_PULLSPEC}" || true
        failures=$((failures+1))
    fi
...
verify_pyxis_write_succeeded() {
    ...
    taskrun_status=$(kubectl get taskrun -n "${managed_namespace}" \
        ...
        -o jsonpath='{.items[0].status.conditions[0].reason}' 2>/dev/null) || taskrun_status=""
    ...
    if [ "${taskrun_status}" != "Succeeded" ]; then
        ...
        taskrun_name=$(kubectl get taskrun -n "${managed_namespace}" \
            -l "tekton.dev/pipelineRun=${pipelinerun_name},tekton.dev/pipelineTask=create-pyxis-image" \
            -o jsonpath='{.items[0].metadata.name}' 2>/dev/null) || taskrun_name=""
...
wait_for_pyxis_indexing_from_cluster() {
    ...
    # Override via PYXIS_REPOSITORY_NAME in test.env if the push target differs across variants.
    local repository_name="${PYXIS_REPOSITORY_NAME:-rhtap/rh-advisories-component}"
