...
# Merge two base64-encoded .dockerconfigjson payloads; keeps the first config and unions auths.
merge_push_docker_configjson_from_b64() {
    local local_b64="${1}" shared_b64="${2}"
    jq -s '. as $configs | $configs[0] | .auths = (($configs[0].auths // {}) * ($configs[1].auths // {}))' \
        <(base64 -d <<< "${local_b64}") <(base64 -d <<< "${shared_b64}")
}

# Merge quay.io/hacbs-release-tests registry auth into push-${component_name}.
# This suite vault still carries redhat-pending Quay creds and Pyxis cert/key; the RPA pushes to
# quay.io/hacbs-release-tests/rh-push-to-external-registry-test (RELEASE-2316).
patch_managed_secrets_after_decrypt() {
    local local_file="${SUITE_DIR}/resources/managed/secrets/managed-secrets.yaml"
    local shared_vault="${SCRIPT_DIR}/push-to-external-registry/vault/managed-secrets.yaml"

    if [[ ! -f "${shared_vault}" ]]; then
        echo "🔴 Shared vault not found at ${shared_vault}" >&2
        return 1
    fi

    (
        set -e
        local xtrace_was_on=0 tmp_shared encoded_file local_b64 shared_b64 merged_json encoded
        ...
        tmp_shared="$(mktemp)"
        encoded_file="$(mktemp)"
        chmod 600 "${tmp_shared}" "${encoded_file}"
        trap 'rm -f "${tmp_shared}" "${encoded_file}"; [[ "${xtrace_was_on}" -eq 1 ]] && set -x' EXIT

        ansible-vault decrypt "${shared_vault}" --output "${tmp_shared}" --vault-password-file "${VAULT_PASSWORD_FILE}"

        local_b64="$(yq '. | select(.metadata.name == "push-${component_name}") | .data.".dockerconfigjson"' \
            "${local_file}")"
        shared_b64="$(yq '. | select(.metadata.name == "push-${component_name}") | .data.".dockerconfigjson"' \
            "${tmp_shared}")"

        if [[ -z "${local_b64}" || "${local_b64}" == "null" ]]; then
            echo "🔴 push-\${component_name} secret missing .dockerconfigjson in ${local_file}" >&2
            exit 1
        fi
        if [[ -z "${shared_b64}" || "${shared_b64}" == "null" ]]; then
            echo "🔴 push-\${component_name} secret missing in push-to-external-registry vault" >&2
            exit 1
        fi

        merged_json="$(merge_push_docker_configjson_from_b64 "${local_b64}" "${shared_b64}")"
        unset local_b64 shared_b64
        encoded="$(printf '%s' "${merged_json}" | base64 -w 0)"
        unset merged_json
        printf '%s' "${encoded}" > "${encoded_file}"
        unset encoded

        # eval-all: in-place update must not filter documents (plain select() would drop other secrets).
        yq -i eval-all \
            '(. | select(.metadata.name == "push-${component_name}") | .data[".dockerconfigjson"]) = load_str("'"${encoded_file}"'")' \
            "${local_file}"
        if ! yq -e '. | select(.metadata.name == "push-${component_name}") | .data.".dockerconfigjson"' "${local_file}" >/dev/null \
            || ! yq -e '. | select(.metadata.name == "pyxis-${component_name}") | .data.cert' "${local_file}" >/dev/null \
            || ! yq -e '. | select(.metadata.name == "pyxis-${component_name}") | .data.key' "${local_file}" >/dev/null; then
            echo "🔴 managed secrets patch removed push-\${component_name} or pyxis-\${component_name} document" >&2
            exit 1
        fi
        echo "Merged hacbs-release-tests Quay credentials into push-\${component_name} secret."
    )
}
...
# Invoked from cleanup_resources on script EXIT (see run-test.sh).
suite_exit_cleanup() {
    cleanup_pyxis_credentials
}

# Cleanup Pyxis credential files (idempotent - safe to call multiple times)
# Returns 0 when both deletes succeed or files are already gone; 1 if any rm failed.
cleanup_pyxis_credentials() {
    local failures=0 cert_file="${PYXIS_CERT_FILE:-}" key_file="${PYXIS_KEY_FILE:-}"

    if [ -n "${cert_file}" ] && [ -f "${cert_file}" ]; then
        rm -f "${cert_file}" || failures=$((failures + 1))
    fi
    if [ -n "${key_file}" ] && [ -f "${key_file}" ]; then
        rm -f "${key_file}" || failures=$((failures + 1))
    fi
...
# Sets global PYXIS_CERT_FILE and PYXIS_KEY_FILE (removed on EXIT via suite_exit_cleanup)
setup_pyxis_credentials() {
    local cert_secret_encoded key_secret_encoded
    local xtrace_was_on=0

    # CI often runs scripts under xtrace (bash -x). Disable tracing while reading
    # and decoding Pyxis cert/key so those values are not printed.
    [[ "${-}" == *x* ]] && xtrace_was_on=1
    set +x
    trap 'if [[ "${xtrace_was_on}" -eq 1 ]]; then set -x; fi; trap - RETURN' RETURN

    # Vault uses push-${component_name} / pyxis-${component_name} placeholders; envsubst runs at apply time.
    cert_secret_encoded="$(yq -e '. | select(.metadata.name == "pyxis-${component_name}") | .data.cert' \
        "${SUITE_DIR}/resources/managed/secrets/managed-secrets.yaml")" || {
        echo "🔴 Could not find .data.cert in secret pyxis-\${component_name}" >&2
        return 1
    }
    key_secret_encoded="$(yq -e '. | select(.metadata.name == "pyxis-${component_name}") | .data.key' \
        "${SUITE_DIR}/resources/managed/secrets/managed-secrets.yaml")" || {
        echo "🔴 Could not find .data.key in secret pyxis-\${component_name}" >&2
        return 1
    }

    # Validate we got exactly one value (no newlines from multiple matches)
    if [[ "${cert_secret_encoded}" == *$'\n'* ]]; then
        echo "🔴 Multiple matches for secret pyxis-\${component_name} - check managed-secrets.yaml" >&2
        return 1
    fi
