...
#   COMPONENT_NAME_PREFIX  : Prefix for Konflux component names (default: ls)
#                            component name : {PREFIX}-v{ver}-{base}-{nn}
#                              e.g. multi-v4-15-apiserver-watcher-01
#                            GitHub repo name: {COMPONENT_REPO_PREFIX}-{component_name}
#                              e.g. img-pool-ls-v4-15-apiserver-watcher-01
#   BUILD_ARCH_MODE        : Architecture mode (default: multi)
#                            - multi:  docker-build-multi-platform-oci-ta (amd64/arm64/s390x/ppc64le)
#                            - single: docker-build-oci-ta (amd64 only, ~4x faster)
#   DISABLE_QUAY_REUSE     : Control Quay image search for invalid/zombie components (default: false)
#                            - false: Search Quay for :stable tag, reuse digest if found
#                            - true:  Skip Quay lookup, always trigger a fresh build
#   COMPONENT_REPO_PREFIX  : Fixed prefix prepended to the component name to form the GitHub
#                            repo name (default: img-pool). Normally left unchanged.
...
#     - Then waits BATCH_DELAY seconds before issuing next pair (prevents PAC/infra overload)
#       Single-arch default: 180s (3 min)  |  Multi-arch default: 600s (10 min)
#       Override: export BATCH_DELAY=<seconds>
#   
#   Examples (single-arch, BUILD_ARCH_MODE=single, 5 per batch, 3min inter-batch delay):
#     - 50 components: ~35 minutes  (10 batches × 3min = 30min pauses + ~5min final builds)
#     - 100 components: ~65 minutes  (20 batches × 3min = 60min pauses + ~5min final builds)
#     - 200 components: ~125 minutes (40 batches × 3min = 120min pauses + ~5min final builds)
#   Examples (multi-arch, BUILD_ARCH_MODE=multi, 10min inter-pair delay):
...
# - false: Search Quay first for :stable tag, reuse digest if found, rebuild if not (default)
# - true:  Skip Quay lookup, always trigger a fresh build for missing/invalid components
...
# Default: "ls" → ls-v4-15-apiserver-watcher-01
COMPONENT_NAME_PREFIX="${COMPONENT_NAME_PREFIX:-ls}"
...
BUILD_ARCH_MODE="${BUILD_ARCH_MODE:-single}"
...
# Each component gets its own repo: {COMPONENT_REPO_ORG}/{COMPONENT_REPO_PREFIX}-{component_name}
# e.g. hacbs-release-tests/img-pool-ls-v4-15-apiserver-watcher-01
...
check_quay_image_exists() {
    ...
    if [ "$http_code" = "200" ]; then
        ...
        # Prefer the :stable tag (added by Konflux on every successful build via
        # LABEL konflux.additional-tags="stable"), falling back to any other real tag.
        ...
        manifest_digest=$(echo "$tags_response" | jq -r '
            ...
            | (map(select(.name == "stable")) + map(select(.name != "stable")))
            | .[0].manifest_digest // ""' 2>/dev/null)
...
# create_pac_scm_secret: Creates the per-component SCM credential secret that tells PAC
# to authenticate with GitHub using our PAT instead of the GitHub App installation.
# Without this secret (type basic-auth + appstudio.redhat.com/credentials: scm label),
# PAC falls back to the GitHub App and fails with Error 70 if the App is not installed
# for the repository.
create_pac_scm_secret() {
    local component_name="$1"
    local repo_name="${COMPONENT_REPO_PREFIX}-${component_name}"
    local secret_name="pipelines-as-code-secret-${repo_name}"
...
metadata:
  ...
  annotations:
    appstudio.redhat.com/scm.repository: ${COMPONENT_REPO_ORG}/${repo_name}
type: kubernetes.io/basic-auth
stringData:
  password: ${GITHUB_TOKEN}
EOF
    log_info "   ✓ PAC SCM credential secret: ${secret_name}"
}
...
wait_for_pac_and_merge_pr() {
    ...
    local max_wait=300  # 5 minutes (PAC webhook setup can be slow after branch creation)
    local wait_time=0
    ...
    while [ $wait_time -lt $max_wait ]; do
        ...
        if [ "$pac_state" = "enabled" ]; then
            ...
            # Check for PAC configuration errors in events
            ...
            if [ -n "$merge_url" ]; then
                ...
                if [ -n "$pr_number" ]; then
                    ...
                    # Clean PR branch before merging
                        ...
                        if [ "$clone_success" = "true" ]; then
                            ...
                            for push_file in .tekton/*push*.yaml; do
                                if [ -f "$push_file" ]; then
                                    ...
                                    # Downgrade buildah-oci-ta to 0.7 if a newer version is present.
                                    # 0.8+ added mandatory in-task cosign signing via TUF
                                    # (tuf.trusted-artifact-signer.svc), which is not reliably
                                    # available on staging. 0.7 skips in-task signing; Tekton Chains
                                    # still provides SLSA provenance and signatures externally.
                                    if grep -q "task-buildah-oci-ta:0\.[89]" "$push_file" 2>/dev/null; then
                                        sed -i 's|task-buildah-oci-ta:0\.[89][^@]*@sha256:[a-f0-9]*|task-buildah-oci-ta:0.7@sha256:cde309fc09b68c39ca283c4b121ddf99d900e3ff2cf3d5e7951cb707eeeddfae|g' "$push_file"
                                        file_modified=true
                                        log_info "      📌 Pinned buildah-oci-ta to 0.7 (TUF not stable on staging)"
                                    fi
                                    ...
                                    if [ "${BUILD_ARCH_MODE}" = "multi" ]; then
                                        ...
                                        # single-arch: strip any multi-platform annotations that may have been
                                        # carried over from the template (pre-pushed .tekton/push.yaml has them).
                                        if grep -q "multi-platform-required\|request-platforms" "$push_file" 2>/dev/null; then
                                            yq eval -i 'del(.metadata.annotations."build.appstudio.openshift.io/multi-platform-required")' "$push_file"
                                            yq eval -i 'del(.metadata.annotations."build.appstudio.openshift.io/request-platforms")' "$push_file"
                                            yq eval -i '(.spec.params[] | select(.name == "build-platforms") | .value) = ["linux/x86_64"]' "$push_file" 2>/dev/null || true
                                            yq eval -i '(.spec.pipelineSpec.params[] | select(.name == "build-platforms") | .default) = ["linux/x86_64"]' "$push_file" 2>/dev/null || true
                                            file_modified=true
                                        fi
                                    fi
                            ...
                            # Remove any on-pull-request .tekton files PAC auto-generated.
                            # We only want push-triggered builds; PR builds create noise (cancelled runs).
                            for pr_file in .tekton/*pull-request*.yaml .tekton/*on-pr*.yaml; do
                                if [ -f "$pr_file" ]; then
                                    git rm -q "$pr_file" 2>/dev/null && \
                                        log_info "      🗑️  Removed pull-request trigger: $(basename "$pr_file")" && \
                                        pr_modified=true
                                fi
                            done
...
ensure_component_repo() {
    ...
    if [ "$repo_exists" = false ]; then
        ...
        # Push ONLY Dockerfile (no .tekton/) to avoid conflicts with PAC auto-generation
        if [ -d "$template_dir" ] && [ -f "${template_dir}/Dockerfile" ]; then
            ...
            # Only push Dockerfile in the initial commit — no .tekton/push.yaml.
            # PAC generates its own ${component_name}-push.yaml via configure-pac.
            # Pre-pushing a push.yaml causes duplicate PipelineRun names (PAC rejects them).
            local init_size_mb=$((100 + RANDOM % 401))
            sed "s|__IMAGE_SIZE_MB__|${init_size_mb}|g" \
                "${template_dir}/Dockerfile" > "$tmp_push/Dockerfile"
    ...
    if git clone -q --depth 1 --branch "${component_branch}" \
        ...
        if [ "$cleanup_only" = "true" ]; then
            ...
            # Generate custom Dockerfile with random size (100–500 MB)
            # Kept under 500 MB to avoid OOM kills on remote multi-arch build hosts
            # (arm64/s390x/ppc64le hosts typically have 2-4 GB RAM; buildah needs
            # to hold the uncompressed layer in memory during commit).
            size_mb=$((100 + RANDOM % 401))  # Random 100-500 MB
...
DOCKERFILE_EOF
            ...
            # Substitute the placeholder with the actual random size
            sed -i "s|__IMAGE_SIZE_MB__|${size_mb}|g" Dockerfile
...
# Fetch all components and pipelineruns once for batch checking
...
# Filter to only components matching the current COMPONENT_NAME_PREFIX so that
# runs with different prefixes (e.g. "test" vs "multi") are fully isolated from
# each other — prevents false counts and accidental cross-prefix deletions.
ALL_COMPONENTS_JSON=$(echo "${ALL_COMPONENTS_JSON}" | jq \
    --arg prefix "${COMPONENT_NAME_PREFIX}-" \
    '{items: [.items[] | select(.metadata.name | startswith($prefix))]}')
...
# Fetch all component repos once (one repo per component, OCP-style)
...
for page in 1 2 3 4 5; do
    ...
    raw_count=$(echo "${page_raw}" | grep -c . 2>/dev/null) || raw_count=0
    if [ "${raw_count}" -lt 100 ]; then
        break
    fi
done
log_info "  Found $(echo "$ALL_COMPONENT_REPOS" | grep -c "^${COMPONENT_REPO_PREFIX}-" 2>/dev/null || echo 0) component repos (prefix: ${COMPONENT_REPO_PREFIX}-)"
...
if [ ${EXISTING_COMPONENT_COUNT} -gt 0 ]; then
    echo "" >&2
    log_info "📊 Existing components in namespace (prefix: ${COMPONENT_NAME_PREFIX}-):"
...
BATCH_SIZE=${BATCH_SIZE:-5}              # Components per batch (5 push-triggered builds per cycle)
# Inter-batch delay: single-arch builds finish in ~5-8 min, multi-arch in ~15-20 min.
# Override via BATCH_DELAY env var.
if [ -n "${BATCH_DELAY:-}" ]; then
    : # use caller-supplied value
elif [ "${BUILD_ARCH_MODE}" = "single" ]; then
    BATCH_DELAY=180                      # 3 minutes for single-arch
else
    BATCH_DELAY=600                      # 10 minutes for multi-arch
fi

# Calculate how many components to create this run
# Only count components WITH builds toward target (zombies don't count)
SKIP_CREATION=false
...
while IFS=: read -r app_name component_count; do
    ...
    for (( i=1; i<=component_count; i++ )); do
        ...
        # Count only components with SUCCESSFUL builds, not zombies or in-progress!
        ...
        if [ "${has_existing_build}" = "true" ]; then
            ...
            # If the component exists in the cluster (found via ALL_COMPONENTS_JSON), just clean up the repo.
            # If it was found only via Quay (k8s Component was deleted), recreate it with the Quay digest
            # so it appears in kubectl / quick-image-report and counts toward the pool.
            component_in_cluster=$(echo "${ALL_COMPONENTS_JSON}" | jq -r \
                ".items[] | select(.metadata.name == \"${component_name}\") | .metadata.name" 2>/dev/null || true)

            # Ensure k8s Component exists and has lastPromotedImage set.
            # Two sub-cases:
            #   A) Component missing from cluster entirely (deleted externally)
            #   B) Component exists but lastPromotedImage is empty (reuse path ran but didn't patch status)
            # In both cases we need the Quay digest.
            needs_quay_patch=false
            if [ -z "${component_in_cluster}" ]; then
                needs_quay_patch=true   # Case A: must create + patch
            elif [ -z "${promoted_image}" ]; then
                needs_quay_patch=true   # Case B: exists but no promoted image
            fi

            if [ "${needs_quay_patch}" = "true" ]; then
                # Get Quay digest if not already available
                if [ -z "${QUAY_IMAGE_DIGEST:-}" ]; then
                    QUAY_IMAGE_DIGEST=""
                    check_quay_image_exists "${component_name}" "${NAMESPACE}" || true
                fi

                if [ -n "${QUAY_IMAGE_DIGEST:-}" ]; then
                    if [ -z "${component_in_cluster}" ]; then
                        log_info "   🔧 Component missing from cluster — recreating with Quay image: ${component_name}"
                        create_pac_scm_secret "${component_name}"
...
spec:
  ...
  secret: pipelines-as-code-secret-${COMPONENT_REPO_PREFIX}-${component_name}
...
EOF
                        quay_reused_count=$((quay_reused_count + 1))
                    else
                        log_info "   🔧 Patching missing lastPromotedImage for: ${component_name}"
                    fi
                    # Patch lastPromotedImage so it counts as valid in the cluster
                    kubectl patch component "${component_name}" -n "${NAMESPACE}" \
                        --subresource=status --type=merge \
                        -p "{\"status\":{\"lastPromotedImage\":\"${QUAY_IMAGE_DIGEST}\"}}" &>/dev/null || true
                    log_info "   ✓ lastPromotedImage set: ${QUAY_IMAGE_DIGEST}"
                else
                    log_warning "   ⚠️  No Quay image found for missing component — will rebuild: ${component_name}"
                    has_existing_build=false
                fi
            fi
            ...
            # else: fall through to normal creation below
        fi
        ...
        if kubectl get component "${component_name}" -n "${NAMESPACE}" &>/dev/null; then
            ...
            if [ "${DISABLE_QUAY_REUSE:-false}" = "true" ]; then
                ...
                quay_check_result=0
                check_quay_image_exists "${component_name}" "${NAMESPACE}" || quay_check_result=$?
            fi
            ...
            if [ "$quay_check_result" -eq 0 ]; then
                ...
                # Pre-create SCM credential secret so PAC uses our PAT instead of the GitHub App
                create_pac_scm_secret "${component_name}"
...
EOF
                ...
                # Cannot verify Quay (403/404/network error).
                # The component is already a zombie (no lastPromotedImage), so keeping it
                # would leave it in COMPONENT_LIST with no usable image - triggering a
                # failure at digest extraction. Rebuild instead.
                log_warning "   ⚠️  Cannot verify Quay for zombie ${component_name} (HTTP 403/404 or network error) - will rebuild"
                force_delete_component "${component_name}" "${NAMESPACE}"
                quay_already_checked=true
                cannot_verify_count=$((cannot_verify_count + 1))
                # Fall through to component creation / rebuild below
            else
            ...
            # Check if PAC has errors (no common history, failed PRs, etc.)
...
EOF
        ...
        # Pre-create SCM credential secret so PAC uses our PAT instead of the GitHub App.
        # Without this, PAC falls back to the GitHub App and fails with Error 70.
        create_pac_scm_secret "${component_name}"
        
        # No configure-pac here - add it only after the GitHub branch is ready
        # (adding it now would cause PAC to fail immediately since the branch doesn't exist yet)
...
EOF
        ...
        if ! ensure_component_repo "${component_name}"; then
            ...
            if wait_for_pac_and_merge_pr "${component_name}" "${component_repo_name}"; then
                ...
                # Step 5: Wait for build to start (webhook may need ~60s to register after PAC PR merge)
                build_started=false
                for build_check_iter in {1..20}; do  # Wait up to 60s (20 * 3s)
                ...
                if [ "$build_started" = "false" ]; then
                    log_warning "   ⚠️  No build after 60s - pushing trigger commit to activate webhook..."
                    # PAC webhook may not have been ready at merge time; push a new commit to fire it
                    _trigger_repo="${COMPONENT_REPO_PREFIX}-${component_name}"
                    _trigger_branch="${COMPONENT_BRANCH_PREFIX}-${component_name}"
                    _trigger_size=$((100 + RANDOM % 401))
                    _trigger_tmp="${TEMP_DIR}/trigger-${component_name}"
                    rm -rf "$_trigger_tmp"
                    if git clone -q --depth 1 --branch "$_trigger_branch" \
                            "https://${GITHUB_TOKEN}@github.com/${COMPONENT_REPO_ORG}/${_trigger_repo}.git" \
                            "$_trigger_tmp" 2>/dev/null; then
                        cd "$_trigger_tmp"
                        git config user.name "Large Snapshot Test Bot"
                        git config user.email "release-team@redhat.com"
                        # Substitute placeholder so Dockerfile is always valid
                        sed -i "s|ARG IMAGE_SIZE_MB=.*|ARG IMAGE_SIZE_MB=${_trigger_size}|" Dockerfile 2>/dev/null || \
                            echo "SIZE_TRIGGER=${_trigger_size}" >> .build-trigger
                        git add -A &>/dev/null
                        if git diff --cached --quiet 2>/dev/null; then
                            echo "BUILD_TRIGGER=$(date +%s)" >> .build-trigger
                            git add .build-trigger &>/dev/null
                        fi
                        if git commit -q -m "build: Trigger webhook build (${_trigger_size}MB) for ${component_name}" 2>/dev/null && \
                           git push -q origin "$_trigger_branch" 2>/dev/null; then
                            log_info "   ✓ Trigger commit pushed — build should start shortly"
                        else
                            log_warning "   ⚠️  Failed to push trigger commit for ${component_name}"
                        fi
                        cd - >/dev/null
                        rm -rf "$_trigger_tmp"
                    else
                        log_warning "   ⚠️  Could not clone repo to push trigger commit"
                    fi
                fi
            else
        ...
        # Batch delay: pause after every BATCH_SIZE components to let builds start before queuing more
        if [ $((actual_new_count % BATCH_SIZE)) -eq 0 ] && [ $actual_new_count -gt 0 ]; then
            ...
            log_info "   ⏳ Waiting ${BATCH_DELAY}s for builds to complete before next pair..."
            sleep ${BATCH_DELAY}
    ...
    # Count only VALID components (WITH successful builds), not zombies!
