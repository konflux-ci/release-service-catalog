...
create_kubernetes_resources() {
    ...
    local max_retries=5
    local attempt
    # apply is required for retries: a ResourceQuota 409 can fail after some objects
    # already exist, and create then returns AlreadyExists for the rest of the file.
    for attempt in $(seq 1 "${max_retries}"); do
        if kubectl apply -f "${tmpDir}/tenant-resources.yaml" -n "${tenant_namespace}"; then
            break
        fi
